...
public class StravaAthleteQuery {
    ...
    public List<StravaAthleteBikeResult> getAthleteBikes() {
        ...
        List<StravaAthleteBikeResult> results = new ArrayList<>();
        models.forEach(bike -> {
            StravaAthleteBikeDetail result = stravaApiAdapter.getBikeDetail(bike.getId(), connection.getAccessToken());
            results.add(mapper.toResult(result));
        });
        return results;
    }
}